...
cat <<'EOF' > ~/troubleshooting/solution-9/provider.tf
terraform {
  required_providers {
    docker = {
...
cat <<'EOF' > ~/troubleshooting/solution-9/variables.tf
...
cat <<'EOF' > ~/troubleshooting/solution-9/main.tf
...
cat <<'EOF' > ~/troubleshooting/solution-9/locals.tf
locals {
...
mkdir -p ~/troubleshooting/solution-9/files
cat <<'EOF' > ~/troubleshooting/solution-9/files/example.txt
Tofu Exercises
EOF